...
cl_device_id Device::getId() const {
  return device;
}
...
std::string Device::queryName() const {
  return DeviceInfoTraits<std::string>::getDeviceInfo(
                                        device,
                                        CL_DEVICE_NAME);
}
...
returnType DeviceInfoTraits<returnType>::getDeviceInfo(
...
size_t getDeviceInfoSize(cl_device_id deviceId, 
                         cl_device_info deviceInfoName) {
  size_t result;
  cl_int errorCode = clGetDeviceInfo(deviceId, deviceInfoName,
                                     0, NULL, &result);
...
std::string DeviceInfoTraits<std::string>::getDeviceInfo(
                                           cl_device_id deviceId,
                                           cl_device_info deviceInfoName) {
